...
import DitheringTool from '../../components/curriculum/curriculum-images/dithering_tool.png';
import DuplicateSprite from '../../components/curriculum/curriculum-images/duplicate_sprite.gif';
import MakeEyesMove from '../../components/curriculum/curriculum-images/make_eyes_move.gif';
import EyeAnimation from '../../components/curriculum/curriculum-images/speed_of_eyes.gif';
import ExportAnimation from '../../components/curriculum/curriculum-images/export_sprite.gif';
import UploadSprite from '../../components/curriculum/curriculum-images/upload_sprite.gif';
import FlipSprite from '../../components/curriculum/curriculum-images/flip_sprite.gif';
    ...
    return(
      <div>
        ...
        <p>Show them tools on the left. You wont need to demonstrate every single tool since most of them would 
            likely know most of them. Encourage them to experiment to learn all of the tools available. Here are 
            the tools I would encourage to demonstrate:</p>
            <ul>
                <li>On top left, under PISKEL, there are four different sized squares. They are brush sizes, 
                    useful to make your work faster.</li>
                    <img src={SizesOfSquares} alt="There are four different sizes of squares" />
                <li>At the bottom of the right column of the tool set is a dithering tool. Try it out, it’s kinda cool. </li>
                    <img src={DitheringTool} alt="Dithering Tool" />
                </ul>
        <p>Once they are done drawing the character; tell them to start animating it. First we duplicate our frame. 
            Hover over the frame (the yellow highlighted thing, and then duplicate icon with two papers next to 
            each other will appear. </p>
        ...
        <p>After both of the eyes are ready, they can look at what they created! Make sure in the right top corner 
            the click full size, so they can see it. Explain what FPS(frames per second) means, and make sure to 
            lower it to 2 or 3 FPS. (it’s the scrollbar under the preview) </p>
        ...
        <p>Duplicate the first frame, and using the transform tools on the right, flip the image vertically. If the 
            ghost is in the center, there won’t be any problems; however, if its off, the ghost will flicker. To fix 
            this we will need to move the ghost where he was in the last frame. This is where the onion tool is
             helpful. To the left of the FPS bar is an onion icon, click it, and you should be able to see where 
             the ghost was last time. </p>
        <img src={FlipSprite} height="300" alt="Duplicate your current sprite and then flip the new one" />
        <p>After that let them work on their own stuff. If they want to, they can make the ghost’s wiggles wiggle. </p>
        ...
        <p>Make the scale 6x or higher so the images do not show up tiny in scratch and then click download zip
            (if the kids create more than 10 frames, might be easier to export in gif, since it keeps everything in order) </p>
